...
class Detector(caffe.Net):
	...
	# Bricolage de bledard pour virer les boxes foireuses

	#window_inputs = [x for x in window_inputs if x is not None]
    ...
    def crop(self, im, window):
	    ...
	    #f1=open('file.sh', 'a+')
            ...
            # POUR VIRER LES BOXES FOIREUSES 
            if(context_crop.size != 0):
            	with open('file2.csv', 'a+') as myfile:
