...
im = plt.imread('../images/pedestrians.jpg')
plt.imshow(im)
currentAxis = plt.gca()
...
# Let's take all 'person' detections and NMS them to get rid of overlapping windows.
scores = predictions_df['car']
windows = df[['xmin', 'ymin', 'xmax', 'ymax']].values
dets = np.hstack((windows, scores[:, np.newaxis]))
nms_dets = nms_detections(dets)

# Show top 3 NMS'd detections for 'person' in the image and note the gap between the top scoring box (red) and the remaining boxes.
plt.imshow(im)
currentAxis = plt.gca()
